...
- 新增京东订单、银行支付明细和京东数电发票识别，并支持从发票备注中提取订单号进行匹配
- 新增闲鱼订单及对应支付宝账单识别
- 新增美团订单和美团账单识别，并兼容发票金额不包含配送费的情况
- 将自动匹配扩展为“订单 + 支付明细 + 发票”组合，并补充京东、银行、闲鱼和美团来源图标

### 🐞 修复

- 修复部分包含异常 ULC 对象头的电子发票无法正常预览和导出的问题（#5）
- 修复金额包含千分位分隔符时无法正确提取的问题，例如 `12,345.67`

### 🔧 工程化

- 调整 CI 构建配置，使 Windows 和 Linux 检查无需发布签名私钥即可验证安装包构建
